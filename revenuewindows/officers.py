...
class Ui_Dialog(object):
    def loginCheck(self):
        user_name=self.uname.text()
        pass_word=self.pswd.text()
        
        connecction=sqlite3.connect("login.db")
        result=connecction.execute("select * from officers where Username=? and Password=?",(user_name,pass_word))
